...
export default async function handler(req,res){
  ...
  try{
    ...
    const isBooking=e=>{
      const hasGuest=e.att.some(a=>a.email&&a.email!==OWNER);
      const t=((e.summary||'')+' '+(e.description||'')).toLowerCase();
      return hasGuest||/consult|prenotaz|cartoman|numerolog/.test(t);
    };
    const out=parseICS(txt).filter(e=>e.start&&!e.start.allday&&e.start.ms>=from&&e.start.ms<=to&&e.status!=='CANCELLED'&&isBooking(e)).map(e=>{
